...
import type { TaskWithRelations, TaskFilters as FilterType } from "@/types/task"
import { TaskFilters } from "@/components/tasks/task-filters"

interface TaskListViewProps {
    initialTasks: TaskWithRelations[]
    workspaceId: string
    workspaceSlug?: string // Make optional or required, but usually passed
    currentUserRole?: string | null
}

export function TaskListView({ initialTasks, workspaceId, workspaceSlug: propSlug, currentUserRole }: TaskListViewProps) {
    const params = useParams()
    // Use prop if available, fallback to params
        ...
        <div className="flex flex-col h-[calc(100vh-8rem)]">
            ...
            <div className="flex flex-col md:flex-row items-center justify-between p-4 border-b bg-background sticky top-0 z-10 gap-4">
                ...
                <div className="flex items-center gap-2 w-full md:w-auto md:ml-auto">
                    {/* Only show Create button if canEdit? Usually handled inside form or by hiding button */}
                ...
                <div className="flex-1 overflow-auto p-4 space-y-1 pb-20 md:pb-4">
                    {filteredTasks.length === 0 ? (
                        <EmptyState
                            ...
                            actionLabel={!searchQuery && ["OWNER", "ADMIN", "MEMBER"].includes(currentUserRole || "") ? "Create Task" : undefined}
                        />
                    ) : (
            ...
            {view === "board" && (
                <div className="flex-1 overflow-hidden p-4 pb-20 md:pb-4">
                    <div>Board View Temporarily Disabled</div>
